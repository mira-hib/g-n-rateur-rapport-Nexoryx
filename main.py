...
from agents.report_workflow import ReportGenerationWorkflow
...
def list_audits():
    """Liste tous les audits disponibles."""
    print("\n=== Audits disponibles ===\n")
...
def generate_report(audit_id: int, output_format: str) -> bool:
    """
    Génère un rapport pour un audit donné.
    ...
    if audit_data is None:
        print(f"L Erreur: Audit #{audit_id} introuvable.")
        return False
    ...
    # Générer le rapport via le workflow
    ...
    # Générer le(s) fichier(s) de sortie
    ...
    try:
        ...
        print(f"L Erreur lors de la création du fichier: {e}")
        import traceback
        traceback.print_exc()
        return False
  ...
  Créez un fichier .env à la racine avec vos clés API:
    ...
    if args.audit_id:
        ...
        if not settings.openai_api_key and not settings.anthropic_api_key:
            print("\nL Erreur: Aucune clé API configurée!")
